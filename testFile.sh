...
# 	fi
# }
error_message="dpkg: error processing package [packaged-name] (--configure): package is in a very bad inconsistent state"
package=$(echo $error_message | grep -o 'package.*')
echo $package
package_name=$(echo $error_message | sed 's/dpkg: error processing package \(.*\) (--configure).*/\1/')
echo $package_name